...
class Dataset:
  ...
  def read_rating_matrix(self, rating_matrix_csv: str):
    """評価値行列を読み込む。
  ...
  def to_movies_movies_similarity_matrix(self, movies_movies_similarity_matrix_csv: str):
  ...
  def read_movies_movies_similarity_matrix(self, movies_movies_similarity_matrix_csv: str):
    """映画-映画類似度行列を読み込む。
  ...
  def to_atomic(self, dataset_name: str):
    """データセットをRecBole用のAtmic File形式に変換して保存する。
    ref: https://recbole.io/docs/user_guide/data/atomic_files.html
